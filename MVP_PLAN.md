...
Status: implemented through milestone 5; console sign-in and one Oracle 19c instance are qualified. The plan is kept as
...
**Little here is qualified yet.** Console sign-in has been qualified against Keycloak,
and the full suite has run against one Oracle 19c instance (19.9.1, non-CDB, thin
mode). Claims that need a second database, a PDB, thick mode, a provider, DataForge
or load still rest on the stand-in. [docs/compatibility.md](docs/compatibility.md) records what has and has not
...
| SQL worksheet | Execute one selected statement or complete PL/SQL block, bind parameters, bounded result grid, cancellation, elapsed time, saved scripts, explicit transaction controls | DML remains uncommitted until requested; rollback works; cancellation does not affect another user's execution | Implemented. Cancellation, DDL commit, deadlines and connection loss behaved as designed on one 19c instance, after a deadline crash in the driver was found and fixed - see docs/compatibility.md |
| PL/SQL workspace | Source editor, source diff, compile package specification/body and standalone routines, compiler errors, bounded DBMS_OUTPUT, anonymous test blocks | Seeded invalid package shows correct errors, can be repaired and compiled, and produces expected test output | Implemented. Compile, line-level errors, repair and bounded DBMS_OUTPUT pass on one 19c instance; the output buffer was failing large blocks until fixed |
| Tuning workbench | Explain plan; cached cursor plan when accessible; SQL ID/child cursor selection; current SQL statistics; saved before/after observations | A seeded slow query can be examined and compared under equivalent test conditions; estimated and measured values are clearly distinguished | Implemented. Estimated plans, cached cursors and plan reads pass on one 19c instance |
| DBA overview | Sessions and blockers, tablespace usage, invalid objects, scheduler job status/failures, connection health | Each panel shows collection time and permission/error state; unavailable data is never displayed as healthy | Implemented. Every panel collects on one 19c instance, and the blocking panel is shown a genuinely blocked session. Both scheduler queries were broken until fixed |
| Controlled runbooks | Collect health report, recompile one selected object, gather statistics for one selected table | Mutations show exact target and parameters, require an authorized execution action, and are reported as verified only when the evidence shows the change took effect | Implemented. All three runbooks ran on one 19c instance, and each one's verification requirement is now read rather than only recorded - recompiling the deliberately broken seeded package reports `unverified`, because the object is still INVALID afterwards. Recompile was generating invalid SQL for bodies until fixed |
...
5. Implement authenticated per-target authorization and reviewed Oracle grants. Implemented. The grant script in `oracle/grants/` has been run, except the seven `SYS` view grants: it needs `SYS`, and `SELECT_CATALOG_ROLE` stood in for those.
6. Implement execution records, dedicated worksheet sessions and bounded fetch. Implemented; qualified on one 19c instance.
7. Demonstrate commit, rollback, cancellation, idle expiry and network-failure behavior. Demonstrated against one 19c instance, including idle expiry. Worker restart has no test on either backend.
...
- **Partly met.** Integration tests on Oracle 19c plus the selected development database version; document exact tested versions and configurations. The full suite passes on one 19c instance (19.9.1, non-CDB, thin), recorded in docs/compatibility.md. Not yet run: a PDB, thick mode, the development database version.
...
- **Partly met.** Verify multi-user session isolation, commit/rollback behavior, DDL handling, cancellation, blocked queries, connection loss, worker restart and unknown write outcomes. All but worker restart pass on one 19c instance, with sessions killed for real. Isolation between two databases needs a second target.
- **Partly met.** Verify binds, quoted identifiers, Unicode, NUMBER precision, dates/time zones, nulls and bounded LOB handling. All verified on one 19c instance, after NUMBER precision and LOB previews were fixed. Time zones are a driver limitation: the offset of TIMESTAMP WITH TIME ZONE is lost, and the harness now says so on the result.
...
- **Partly met.** Run end-to-end developer and DBA scenarios with fixtures whose expected results are known. `tests/e2e` runs the six MVP workflows against known fixtures, on the stand-in and on one 19c instance.
